...
using namespace std;
    ...
    bool scanTemplate(string playerNum) {
        ...
        if (notFull == true && fourInARow == false && fourInACol == false && fourInADiag == false) { // no wins / not full
            ...
            initialize();
            cout << "Player " << playerNum << " has won. Play again?" << endl;
            playAgain();
            return false;
        }
    }
    ...
    void playMove(player p) {
        ...
        switch(p) {
            case 'x':
                playerNum = "one"; // translates so that it's easier to print out
                break;
            case 'o':
                playerNum = "two";
                break;
        ...
        cout << "Player " + playerNum + ", choose column to play." << endl;
