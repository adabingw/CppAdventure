...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        gene_prob = PROBS['gene'][gene_id]
        trait_prob = PROBS['trait'][gene_id][trait_id]

        # no parents, rando probability
        if people[person]['mother'] is None and people[person]['father'] is None:
            ...
            for parent in [mother, father]:
                # parent_gene_id = 1 if parent in one_gene else 2 if parent in two_genes else 0
                ...
                if parent_gene_id == 0:
                    trait = 0 + PROBS['mutation']
                elif parent_gene_id == 1:
                    trait = 0.5
                else:
                    trait = 1 - PROBS['mutation']
                traits[parent] = trait

            if gene_id == 0:
                probability *= (1 - traits[mother]) * (1 - traits[father])
            elif gene_id == 1:
                probability *= (1 - traits[mother]) * traits[father] + traits[mother] * (1 - traits[father])
            else:
                # 2, both of parents gave gene
                probability *= traits[mother] * traits[father]
