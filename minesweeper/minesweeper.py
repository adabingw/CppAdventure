...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbouring_cells = self.neighbouring_cells(cell)
        for c1 in neighbouring_cells: 
            if c1 in self.mines: 
                count2 =- 1 
            if c1 not in self.mines or c1 not in self.safes: 
                c.add(c1)
        
        # 3
        nSentence = Sentence(c, count2)
        if len(nSentence.cells) != 0: 
        ...
        for s1 in self.knowledge: 
            for s2 in self.knowledge: 
                if s2.cells.issubset(s1.cells): 
                    nCells = s1.cells - s2.cells 
                    nCount = s1.count - s2.count
                    nSentence = Sentence(nCells, nCount)
                    mines = nSentence.known_mines()
                    safes = nSentence.known_safes()
    ...
    def neighbouring_cells(self, cell):
        ...
        # checks rows in board
        for rows in range(self.height):
            # checks columns in board
            for columns in range(self.width):
                # if distance is less than/equal one and isn't self.cell
                if abs(cell[0] - rows) <= 1 and abs(cell[1] - columns) <= 1 and (rows, columns) != cell:
                    neibouring_cells.add((rows, columns))
        return neibouring_cells
